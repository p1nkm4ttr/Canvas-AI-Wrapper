...
READ_TOOL_CONTENT_POLICIES: dict[str, ReadToolContentPolicy] = {
    ...
    "get_calendar": _fenced("fence_untrusted_inline"),
    "get_course_map": _fenced("fence_untrusted_inline"),
    "get_courses": _deferred(
        "Returns course name/code, term, and the caller's own score; course identity is the documented low-risk exception."
    ),
    ...
    # Study mode (step 3).
    "get_study_context": _fenced("fence_untrusted"),
    "get_file_text": _fenced("fence_untrusted"),
    "search_course_content": _fenced("fence_untrusted_inline"),
    "get_announcement_context": _fenced("fence_untrusted"),
    "read_local_document": _fenced("fence_untrusted"),
    "get_due_reviews": _safe(
        "Returns locally stored practice questions authored by the coach, not raw Canvas content."
    ),
    "list_plan_events": _safe(
        "Returns locally stored plan events authored by the coach/student, not Canvas content."
    ),
    "index_course_files": _safe(
        "Returns only extraction counts and status totals; no Canvas-authored text."
    ),
    # Registered only when STUDENT_WRITE_TOOLS enables the write set.
