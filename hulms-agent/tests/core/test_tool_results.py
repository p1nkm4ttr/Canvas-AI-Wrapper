...
async def test_register_all_tools_marks_existing_validation_failure_as_mcp_error():
    ...
    async with Client(mcp) as client:
        result = await client.call_tool(
            "get_syllabus",
            {"course": 60366, "max_chars": 0},
            raise_on_error=False,
        )

    assert result.is_error is True
    assert "max_chars" in getattr(result.content[0], "text", "")
...
async def test_full_registry_suppresses_only_string_output_schemas():
    ...
    # Upstream also asserted a dict-returning tool (list_conversations) kept its
    # schema; every tool surviving the single-user strip returns str.
