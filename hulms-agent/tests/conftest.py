...
def reset_config_between_tests(monkeypatch):
    ...
    from its environment; later tests that patch env vars would silently read
    stale config.

    Also scrub this project's .env variable names (CANVAS_HOST / CANVAS_TOKEN /
    TIMEZONE, loaded into the environment by load_dotenv at import time): they
    take precedence over the upstream CANVAS_API_URL / CANVAS_API_TOKEN names
    that the inherited tests patch, so leaving them set would leak the real
    local configuration into every test.
    """
    for var in ("CANVAS_HOST", "CANVAS_TOKEN", "TIMEZONE"):
